...
########################## Ensemble Selection ################################
##############################################################################
# With this file, the ensemble selection for each random partion is realized.
# The algorithm used for ensembling is the greedy optimization hill-climbing
# technique with bagging.
# The model prediction lists are loaded from "/Predictions". A data frame
# including DMC measures for each ensemble built from a partition is saved to
# "/Predictions".
...
BES = function(X, Y, b = 10L, p = 0.5, r = 100L){
  ...
  while(i < b){
    i         = i + 1L
    ind       = which(W[i, ] == 1)
    W[i, ind] = W[i, ind] * greedOptc(X[, ind], Y, r)
  }
...
p = 0.5  # Probability that a Model is selected as Part of Sample
r = 500L # How often should be averaged per Ensemble?
...
for(i in 1:length(set)){
	...
	### Run Ensemble selection on Training set -> Evaluate with test and validation set
	...
	train.weight = BES(train.pred, train.bv, b, p, r)
	test.weight  = BES(test.pred,   test.bv, b, p, r)
	...
	val.es.train.pred = sweep(val.pred, MARGIN = 2, STATS = train.weight, FUN = "*") 
	val.es.train.pred = rowSums(val.es.train.pred)

	val.es.test.pred  = sweep(val.pred, MARGIN = 2, STATS = test.weight, FUN = "*") 
	val.es.test.pred  = rowSums(val.es.test.pred)

	# Assess model fit

	if(i == 1){
		AC.val.train = AC(val.es.val.train.pred, val.bv)
		AC.val.test  = AC(val.es.val.test.pred, val.bv)
	}else{
		AC.val.train = c(AC.val.train, AC(val.es.val.pred, val.bv))
		AC.val.test  = c(AC.val.test,  AC(val.es.val.pred, val.bv))
	}

}

saveRDS(AC.train, "EStrain.rds")
saveRDS(AC.test,  "EStest.rds")