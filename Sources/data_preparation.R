...
train <- read.table("DMC2015/DMC_2015_orders_train.txt", header=TRUE, sep="|", stringsAsFactors=FALSE)
test <- read.table("DMC2015/DMC_2015_orders_class.txt", header=TRUE, sep="|",stringsAsFactors=FALSE)
total <- rbind(train,test)

trainingSetIndex <- as.numeric(unlist(read.csv("training_set_index.csv", header=FALSE, col.names=FALSE)))
total$trainingSetIndex <- 0
total$trainingSetIndex[trainingSetIndex] <- 1
total$trainingSetIndex[(nrow(train)+1):nrow(total)] <- -1
...
write.csv(merged, "data_temp/long_6.x.csv", row.names=FALSE)
write.csv(total, "data_temp/dmc_6.x.csv", row.names=FALSE)
# 
# for (var in colnames(total)){
#   print(paste("column",var))
#   print(sum(is.na(total[,var])))
# }
# 
# for (var in colnames(merged)){
#   print(paste("column",var))
#   print(sum(is.na(merged[,var])))
# }
